...
import { App, Button, Layout, Menu, Modal, Space, Typography } from "antd";
...
function Header() {
  ...
  const { isLoggedIn, userInfo, logout, initializeAuth, userToken } = useUserStore();
  ...
  const handleLogout = () => {
    ...
    async function userLogoutAction() {
      if (userToken) {
        await PortalAPI.portalLogout(userToken);
      }
      logout();
      router.push('/');
    }
